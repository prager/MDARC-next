  ...
  modalEl.addEventListener('show.bs.modal', async function (ev) {
    ...
    try {
      ...
      if (formEl && hiddenInput) {
        ...
        formEl.action = '<?= site_url('staff/add-fam') ?>/' + encodeURIComponent(d.id_members ?? '');
      }
      ...
      if (formExistingFam && hiddenParent) {
        ...
        formExistingFam.action = '<?= site_url('staff/add-fam-existing') ?>/' + encodeURIComponent(d.id_members ?? '');
      }
        ...
        kidsBody.innerHTML = kids.map(r => `
          <tr>
            ...
            <td class="text-center">
              <a href="<?= site_url('staff/rem-fam/') ?>${r.id_members}"
